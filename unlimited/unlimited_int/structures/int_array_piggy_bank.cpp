...
bool int_array_piggy_bank::withdraw_one_Node_to_insignificant(int_array_list& deposit_to)
{
...
#if DEBUG_MODE > 0
	if (this->find_inconsistencies() || deposit_to.find_inconsistencies())
		throw "\nThe inconsistency was found in start of function: \"bool int_array_piggy_bank::withdraw_one_Node_to_insignificant(int_array_list& deposit_to)\"";
#endif
	if (this->intarrays.length == 0) { return false; }
	many_bits num_of_ints_withdrawn = this->intarrays.first->value->intarr_len;
	if (deposit_to.intarrays.length == 0)
	...
	deposit_to.num_of_ints += num_of_ints_withdrawn;
#if DEBUG_MODE == 2
	std::cout << "\nFinding inconsistencies in end of function \"bool int_array_piggy_bank::withdraw_one_Node_to_insignificant(int_array_list& deposit_to)\"";
#endif
#if DEBUG_MODE > 0
	if (this->find_inconsistencies() || deposit_to.find_inconsistencies())
		throw "\nThe inconsistency was found in end of function: \"bool int_array_piggy_bank::withdraw_one_Node_to_insignificant(int_array_list& deposit_to)\"";
#endif
	return true;
}
void int_array_piggy_bank::deposit(int_array_list& source)
{
	this->prepend(source);
	if (this->num_of_ints > PIGGY_BANK_MAXIMUM)
		this->obey_PIGGY_BANK_MAXIMUM();
}
int_array_piggy_bank::~int_array_piggy_bank()
{
	this->destroy_and_reset();
}